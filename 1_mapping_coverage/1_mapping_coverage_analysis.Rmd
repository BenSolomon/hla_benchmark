...
```{r}
hla_fasta <- Biostrings::readDNAStringSet(here("references/hla_nuc.fasta"))
```

```{r}
get_hla_id_key <- function(hla_nuc_path = here("references/hla_nuc.fasta")){
...
**Note: file paths for kallisto pipeline have not been refactored for relative paths within the GitHub repo**

### Make sample-specific HLA references

```{r}
#TODO refactor all of this make_reference and kallisto?
### Probably not reasonable given it is sequence based and wouldn't put all that in repo
```
...
# get_kallisto_coverage_stats(here("data/isb/kallisto/alignment"))
```

```{r}
isb_coverage_df <- get_kallisto_coverage_stats(here("data/isb/kallisto/alignment"))
pmid_3p_coverage_df <- get_kallisto_coverage_stats(here("data/pmid30518681/scRNA/kallisto/alignment"))
pmid_bulk_coverage_df <- get_kallisto_coverage_stats(here("data/pmid30518681/bulkRNA/kallisto/alignment"))
...
```{r}
# saveRDS(coverage_df, here("1_mapping_coverage/kallisto_coverage.RDS"))
coverage_df <- readRDS(here("1_mapping_coverage/kallisto_coverage.RDS"))
```
...
all_coverage_segment <- coverage_df %>% 
  ...
  ungroup() %>% 
  mutate(platform = fct_recode(factor(platform), "Bulk" = "bulk")) %>%
  mutate(platform = fct_relevel(platform, "Bulk", "3p", "5p"))
...
```{r}
#TODO incorporate archive/hla_msa.Rmd where entropy RDS was created
...
entropy_plt <- hla_entropy %>% 
  ...
  ggplot(aes(x = position, y = shannon_roll)) + 
  geom_path(size = 1, color = "grey25")+
  facet_grid(. ~ locus, scale = "free_x") +
  theme_bw()+
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(y = "Shannon entropy", x="Position (bp)")
entropy_plt
```
...
path <- here("data/isb/logs/INCOV003-BL/INCOV003-BL_hisat.log")
parse_hisat_log(path)
hisat_total_reads(path)
```

```{r}
log_path <- here("data/isb/arcasHLA/INCOV005-BL.genotype.log")
...
```{r}
p5_samples <- read_tsv(here("data/isb/logs/parallel.log")) %>% 
  separate(Command, into = c(NA, "sample"), sep = " ") %>%
  filter(grepl("-(AC|BL)$", sample)) %>% 
  pull(sample) %>% unique()
p5_hisat_dir <- here("data/isb/logs")
p5_arcas_dir <- here("data/isb/arcasHLA")
...
p3_path <- here("data/pmid30518681/scRNA")
...
p3_hisat_dir <- sprintf("%s/logs", p3_path)
...
bulk_path <- here("data/pmid30518681/bulkRNA")
...
bulk_hisat_dir <- sprintf("%s/logs", bulk_path)
...
efficiency_plt <- bind_rows(
  p5_df %>% mutate(platform = "5p"),
  p3_df %>% mutate(platform = "3p"),
  bulk_df %>% mutate(platform = "Bulk")
) %>%
  filter(grepl("^[ABC]|^D[PQR][AB]1", locus)) %>%
  dplyr::select(sample, locus, platform, contains("efficiency")) %>%
...
plt_fig_main <- plot_grid(
  match_x_axis(target = entropy_plt, source = all_coverage_plt)+
    labs(x="Position (bp)"),
  all_coverage_plt,
  efficiency_plt,
  ncol = 1,
...
```
```{r}
# save_plot("figures/v2/1_sequence_coverage.pdf", plt_fig_main, ncol = 1,nrow = 3, base_asp = 3, base_height = 3)
# save_plot("figures/v2/1_sequence_coverage.pdf", plt_fig_main, base_heigh = 7, base_width = 8)
```
