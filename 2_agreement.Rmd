...
accuracy_df <- readRDS("all_hla_drb345_filtered.RDS") %>% 
  filter(
...
# saveRDS(agreement_df, "data/agreement_df.RDS")
# agreement_df <-  readRDS("data/agreement_df.RDS")
```
...
```{r}
save_plot("figures_pdf/v2/4_agreement.pdf", plt_fig_main, base_height = 3, base_width = 9)
```
...
```{r}
save_plot("figures_pdf/v2/s4_agreement.pdf", plt_fig_supp, base_height = 6, base_width = 9)
```
...
saveRDS(
  listN(plt_t1_t2_diff,
        ...
        plt_ea,
        plt_oa),
  "figures_object/4_agreement_objects.RDS"
)
```
