---
title: "6) Composite HLA genotypes"
output: 
  html_notebook:
    toc: true
    toc_depth: 3
...
```{r}
time_df <- data.frame(sample = isb_samples) %>% 
  mutate(log_path = sprintf("%s/logs/%s/%s_pipeline.log", isb_path,sample,sample)) %>% 
  mutate(data = map(log_path, parse_log_time)) %>% 
  unnest(data)

# saveRDS(time_df, here("6_composite/time_df.RDS"))
```

```{r}
time_df <- readRDS(here("6_composite/time_df.RDS"))
time_df
```


```{r}
gg_runtime(time_df)$data %>% 
  group_by(component) %>% 
  summarise(median_time = median(process_time)*60)
```
...
time_df %>% 
  ...
  pivot_wider(names_from = "component", values_from = "process_time") %>% 
  mutate(PRE_PROCESS = `HISAT-AND-SORT` + `ARCAS-EXTRACT` + `INDEX`,
         AO = PRE_PROCESS + `ARCAS-GENOTYPE` + OPTITYPE,
         AOP = PRE_PROCESS + `ARCAS-GENOTYPE` + OPTITYPE + PHLAT,
         ArcasHLA = PRE_PROCESS + `ARCAS-GENOTYPE`,
         OptiType = PRE_PROCESS + `OPTITYPE`,
         PHLAT = PRE_PROCESS + `PHLAT`) %>% 
  select(sample, ArcasHLA, OptiType, PHLAT, AO, AOP) %>% 
  pivot_longer(-sample, names_to = "component", values_to = "process_time") %>% 
  group_by(component) %>% 
  summarise(median_time = median(process_time)*60)
```

```{r, fig.width=6, fig.height=4}
comptime_plt <- time_df %>% 
  select(sample, component, process_time) %>% 
  pivot_wider(names_from = "component", values_from = "process_time") %>% 
  mutate(PRE_PROCESS = `HISAT-AND-SORT` + `ARCAS-EXTRACT` + `INDEX`,
         AO = PRE_PROCESS + `ARCAS-GENOTYPE` + OPTITYPE,
  ...
  select(sample, ArcasHLA, OptiType, PHLAT, AO, AOP) %>% 
  pivot_longer(-sample, names_to = "component", values_to = "process_time") %>% 
  gg_runtime(include_tasks = c("ArcasHLA", "OptiType", "PHLAT", "AO", "AOP"))
...
# saveRDS(wPhlat_tree_model_list, here("6_composite/wPhlat_tree_model_list.RDS"))
# wPhlat_tree_model_list <-  readRDS(here("6_composite/wPhlat_tree_model_list.RDS"))
```
...
# saveRDS(woPhlat_tree_model_list, here("6_composite/woPhlat_tree_model_list.RDS"))
# woPhlat_tree_model_list <-  readRDS(here("6_composite/woPhlat_tree_model_list.RDS"))
```
...
```{r}
save_plot(here("figures_pdf/v2/s5cde_composite.pdf"), plt_fig_supp, base_height = 9, base_width = 6)
```
