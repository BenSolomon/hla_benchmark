...
all_hla <- readRDS( here("1_mapping_coverage/all_hla_expanded.RDS"))
...
drb345_df
# write_csv(drb345_df, here("3_DRB/isb_drb345_df.csv"))
```
...
drb345_ratio_plt_5p <- gg_drb345_ratio(drb345_df)
drb345_ratio_plt_5p
```
### KNN model
...
# saveRDS(drb_fit, here("3_DRB/drb345_knn_model.RDS"))
```
...
post_tally_plt <- drb_key %>% 
  dplyr::rename("copy_invitro" = copy_number, "copy_predicted" = .pred_class) %>% 
...
pre_filter_contingency_arcas <- all_hla %>%   
  ...
  dplyr::rename(copy_invitro=invitro, copy_predicted=arcasHLA)
...
pre_filter_contingency_hlaminer <- all_hla %>%   
  ...
  dplyr::rename(copy_invitro=invitro, copy_predicted=hlaminer)
...
# saveRDS(drb345_filtered_accuracy, here("3_DRB/drb345_filtered_accuracy.RDS"))
# drb345_filtered_accuracy <- readRDS(here("3_DRB/drb345_filtered_accuracy.RDS"))

drb345_unfiltered_accuracy <- all_hla %>% 
    filter(grepl("^DRB[345]", locus)) %>% 
    calculate_drb345_accuracy(match_drb345_na = score_absent) 

# saveRDS(drb345_unfiltered_accuracy, here("3_DRB/drb345_unfiltered_accuracy.RDS"))
# drb345_unfiltered_accuracy <- readRDS(here("3_DRB/drb345_unfiltered_accuracy.RDS"))
...
# saveRDS(drb345_accuracy_comparison, here("3_DRB/drb345_accuracy_comparison.RDS"))
# drb345_accuracy_comparison <- readRDS(here("3_DRB/drb345_accuracy_comparison.RDS"))
```
...
# saveRDS(drb345_difference_df, here("3_DRB/drb345_difference_df.RDS"))
# drb345_difference_df <- readRDS(here("3_DRB/drb345_difference_df.RDS"))
...
drb345_table <- drb345_accuracy_comparison %>% 
  ...
  mutate(`Field 3-HLAminer Unfiltered` = rlang::na_chr,
         `Field 3-HLAminer Filtered` = rlang::na_chr)
...
# saveRDS(drb345_flextable, here("tables/drb345_flextable.RDS"))
print(drb345_flextable)
```

```{r}
# saveRDS(drb345_accuracy, here("3_DRB/drb345_accuracy.RDS"))
```
...
```{r}
# save_plot(here("figures_pdf/v2/3_DRB.pdf"), plt_fig_main, ncol = 2,nrow = 2, base_asp = 1.3)
```
...
# saveRDS(all_hla_drb345_filtered, here("3_DRB/all_hla_drb345_filtered.RDS"))
# all_hla_drb345_filtered <- readRDS(here("3_DRB/all_hla_drb345_filtered.RDS"))

accuracy_df <- calculate_all_hla_accuracy(all_hla_drb345_filtered, match_drb345_na = F)

# saveRDS(accuracy_df, here("3_DRB/isb_accuracy_drb345_filtered.RDS"))
...
saveRDS(
  ...
  here("figures_object/3_DRB_objects.RDS")
)
...
```{r}
save_plot(here("figures_pdf/v2/s3_drb.pdf"), plt_fig_supp, base_height = 9, base_width = 8)
```
