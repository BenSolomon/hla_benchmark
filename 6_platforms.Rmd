...
sample_metadata <- read_csv("/labs/khatrilab/solomonb/covid/pmid30518681/pmid30518681_samples.csv")
...
### Calculations
```{r}
arcas_log_3p_dir <- "/labs/khatrilab/solomonb/covid/pmid30518681/scRNA/arcasHLA"
...
```{r}
# Apply KNN model
drb_model <- readRDS("drb345_knn_model.RDS")
...
drb345_ROC_3p
drb345_3p_predicted %>% roc_auc(truth = copy_number, .pred_0:.pred_2)
...
drb345_ROC_bulk
drb345_bulk_predicted_pseudo %>% filter(copy_number != "2") %>% roc_auc(truth = copy_number, .pred_0:.pred_2)
...
# saveRDS(all_hla_drb345_3p_filtered, "all_hla_drb345_filtered_pmid_scRNA.RDS")
# saveRDS(all_hla_drb345_bulk_filtered, "all_hla_drb345_filtered_pmid_bulkRNA.RDS")

# all_hla_drb345_3p_filtered <- readRDS("all_hla_drb345_filtered_pmid_scRNA.RDS")
# all_hla_drb345_bulk_filtered <- readRDS("all_hla_drb345_filtered_pmid_bulkRNA.RDS")
```
...
### Success tables
```{r}
calculate_summary_df(success_3p_df) %>% 
...
```{r}
calculate_summary_df(success_bulk_df) %>% 
  flex_summary_hla_accuracy() %>% 
  fontsize(size = 8, part = "all") %>% 
  print(preview = "pptx")
```
...
### Accuracy tables
```{r}
calculate_summary_df(accuracy_3p_df) %>% 
  flex_summary_hla_accuracy() %>% 
  fontsize(size = 8, part = "all") %>% 
  print(preview = "pptx")
```
...
```{r}
calculate_summary_df(accuracy_bulk_df) %>% 
  flex_summary_hla_accuracy() %>% 
  fontsize(size = 8, part = "all") %>% 
  print(preview = "pptx")
```
