...
drb_testing <- 
  ...
  # Add in invitro, original and filtered genotyper predictions by joining
  left_join(
    bind_rows(
...
```{r}
drb345_accuracy <- compare_hla(
  hla_df = drb_testing %>% 
    filter(grepl("BL$", sample)) %>% 
    unite(genotyper, filtered, col = "genotyper", sep = "_"),
...
accuracy_plt <- calculate_summary_df(drb345_accuracy) %>% 
  # Using genotyper and genotyper_2 names to fit into gg_summary_hla_accuracy
  separate(genotyper, into = c("genotyper_2", "genotyper"), sep = "_") %>% 
  mutate(genotyper = fct_recode(genotyper, "Unfiltered" = "FALSE", "Filtered" = "TRUE")) %>% 
  mutate(genotyper2 = reformat_hla_genotyper(genotyper_2)) %>% 
  gg_summary_hla_accuracy(color_label = "Accuracy") + facet_grid(genotyper_2~field) + theme(strip.text.y = element_text(angle = 270))
accuracy_plt
```
...
accuracy_df <- compare_hla(hla_df = all_hla_drb345_filtered, 
            reference = "invitro", 
            exclude_missing = F,
...
accuracy_df %>% 
  ...
  filter(grepl("DRB[345]", locus), field == "field_2", genotyper %in% c("arcasHLA", "hlaminer"))
drb345_accuracy %>% 
  filter(grepl("-BL$", sample)) %>% 
  separate(genotyper, into = c("genotyper", "filtered"), sep = "_") %>% 
