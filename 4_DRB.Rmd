...
drb_testing <- expand.grid(sample = all_hla_expanded %>% pull(sample) %>% unique(), 
  ...
  left_join(
    bind_rows(
      all_hla_expanded %>% filter(genotyper == "invitro" & grepl("^DRB[345]", locus)),
      all_hla_expanded %>% filter(genotyper == "arcasHLA" & grepl("^DRB[345]", locus)) %>% mutate(genotyper = "unfiltered"), 
